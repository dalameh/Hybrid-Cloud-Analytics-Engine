...
Rather than Databricks polling S3 directly — which is expensive and degrades at scale — **Auto Loader consumes SQS messages** to discover new files. This is the native AWS event-driven ingestion pattern and scales to millions of objects without performance impact. Messages accumulate safely in the queue while the cluster is scaling or mid-run, and a **dead-letter queue (DLQ)** captures any message that fails repeated processing, preventing poison records from blocking the queue indefinitely. A visibility timeout of 300 seconds gives the DLT job sufficient time to complete processing before any re-delivery occurs.
