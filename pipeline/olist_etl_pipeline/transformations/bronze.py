...
# -----------------------
# s3://<bucket>/landing/orders/year=/month=/day=/file.parquet
# s3://<bucket>/landing/order_items/year=/month=/day=/file.parquet
# etc.
#
# PIPELINE SETTINGS
# -----------------------------------
...
# 1. CONFIG
# =========================================================
BUCKET = "olist-ecommerce-landing-zone-useast1"
...
def add_bronze_metadata(df: DataFrame) -> DataFrame:
    ...
    Added columns:
    - _ingested_at      : TIMESTAMP_NTZ combining upload_date + current wall-clock time
    - _source_file_path : full source file path from Auto Loader metadata
    """
    return (
        df.select(
            "*",
            ...
            ).cast("timestamp_ntz").alias("_ingested_at"),
